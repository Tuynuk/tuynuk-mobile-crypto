...
import 'package:tuynuk_crypto/src/crypto/crypto_core.dart';
  ...
  receivePort.listen((message) {
    ...
    try {
      if (command == 'encrypt') {
        ...
        final service = FileEncryptionService(pin);
        final decryptedData =
            service.decryptFileSync(encryptedFilePath, outputPath);
...
class FileEncryptionService {
  ...
  Uint8List decryptFileSync(String encryptedFilePath, String outputPath) {
    ...
    final key = AppCrypto.deriveKey(pin);
    final encrypt = pc.PaddedBlockCipherImpl(pc.PKCS7Padding(), pc.AESEngine())
      ..init(
          false,
          pc.PaddedBlockCipherParameters(
            pc.KeyParameter(key),
    ...
    final decrypted = encrypt.process(encryptedBytes);
  ...
  Future<Uint8List> decryptFile(
      String encryptedFilePath, String outputPath) async {
    final receivePort = ReceivePort();
    final isolate =
        await Isolate.spawn(_isolateEntryPoint, receivePort.sendPort);
    final sendPort = await receivePort.first as SendPort;
